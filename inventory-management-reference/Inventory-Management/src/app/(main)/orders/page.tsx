...
export default function OrdersPage() {
    ...
    const executeDeleteOrder = async () => {
        ...
        try {
            await deleteDoc(doc(firestore, "orderRecords", orderToDelete));
            toast({
                title: "Order Deleted",
    ...
    const handleExportSingleOrder = (orderId: string, format: 'csv' | 'pdf') => {
        const orderToExport = enrichedOrders?.find(o => o.id === orderId);
        ...
        if (format === 'csv') {
            exportToCsv([orderToExport], `order_${orderId}.csv`, exportColumns);
        } else {
            exportToPdf([orderToExport], `order_${orderId}.pdf`, exportColumns);
        }
    };
        ...
        return (
            <div className="p-4 border rounded-xl bg-card text-card-foreground shadow-sm flex flex-col gap-3 relative">
                ...
                <div className="flex justify-end gap-2 mt-2">
                    ...
                    <Button variant="destructive" size="sm" onClick={() => confirmDeleteOrder(order.id)}>Delete</Button>
                </div>
            </div>
        );
    };
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <SearchableSelect
                            options={[
                                ...
                                ...uniqueYearsForExport.map(y => ({ value: y, label: y }))
                            ]}
                            value={exportYear}
                        ...
                        <SearchableSelect
                            options={[
                                ...
                                ...Array.from({length: 12}).map((_, i) => {
                                    ...
                                    return { value: m, label: format(new Date(`2000-${m}-01`), 'MMMM') };
                                })
                            ]}
                            value={exportMonth}
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <SearchableSelect
                            options={[
                                ...
                                ...uniqueDealers.map(d => ({ value: d, label: d }))
                            ]}
                            value={exportDealer}
                            onValueChange={setExportDealer}
            ...
            <div className="space-y-4">
                <div className="flex flex-wrap gap-4 items-center">
                    <SearchableSelect
                        options={[
                            { value: 'all', label: 'All Months' },
                            ...uniqueMonths.map(m => ({ value: m, label: format(new Date(m + "-01"), 'MMMM yyyy') }))
                        ]}
                        value={selectedMonth}
