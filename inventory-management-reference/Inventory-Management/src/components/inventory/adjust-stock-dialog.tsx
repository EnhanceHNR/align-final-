                ...
                <div className="grid gap-4 py-4">
                    <div className="space-y-2">
                        <Label htmlFor="useBatch">Select Batch</Label>
                         <SearchableSelect
                            options={sortedStock ? sortedStock.map((stock, index) => ({
                                value: stock.expiryDate,
                                label: `Expires: ${format(new Date(stock.expiryDate), 'PPP')} (${stock.quantity} left)`
                            })) : []}
                            value={useBatchExpiry}
                            onValueChange={setUseBatchExpiry}
                            placeholder="Select a batch to use..."
                            emptyMessage="No stock batches available."
                        />
                    </div>
                    {useBatchExpiry && (
                        <div className="space-y-2">
