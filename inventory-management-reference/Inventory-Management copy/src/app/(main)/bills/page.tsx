...
export default function BillsPage() {
    ...
    const { data: dealers, isLoading: dealersLoading } = useCollection<Dealer>(useMemo(() => firestore ? collection(firestore, 'dealers') : null, []));
    const { data: deliveries, isLoading: deliveriesLoading } = useCollection<Delivery>(useMemo(() => firestore ? collection(firestore, 'deliveries') : null, []));
    const { data: orders, isLoading: ordersLoading } = useCollection<PurchaseOrder>(useMemo(() => firestore ? collection(firestore, 'orderRecords') : null, []));
    ...
    const handleGenerateStatement = async () => {
        if (!firestore || selectedStatementBills.length === 0) {
            toast({ variant: 'destructive', title: 'No bills selected' });
            return;
        }

        const dealerId = selectedStatementBills[0].dealerId;
        const isSingleDealer = selectedStatementBills.every(b => b.dealerId === dealerId);

        if (!isSingleDealer) {
            toast({ variant: 'destructive', title: 'Multiple Dealers', description: 'Please select bills from only one dealer to generate a statement.' });
            return;
        }
        ...
        const totalAmount = selectedStatementBills.reduce((sum, b) => sum + b.actualPrice, 0);

        const earliestDate = selectedStatementBills.reduce((earliest, b) => {
            const billDate = new Date(b.deliveryDate);
            return billDate < earliest ? billDate : earliest;
        }, new Date());

        const latestDate = selectedStatementBills.reduce((latest, b) => {
            const billDate = new Date(b.deliveryDate);
            return billDate > latest ? billDate : latest;
        }, new Date('1970-01-01'));
        
        const newStatementId = doc(collection(firestore, 'statements')).id;
        const deliveryIds = selectedStatementBills.map(b => b.id);
        ...
        const statementRef = doc(firestore, 'statements', newStatementId);
        
        batch.set(statementRef, newStatementData);

        const orderIdsToUpdate = new Set(selectedStatementBills.map(b => b.orderRecordId));
        orderIdsToUpdate.forEach(orderId => {
            const orderRef = doc(firestore, 'orderRecords', orderId);
            batch.update(orderRef, { paymentStatus: 'Pending Statement' });
        });
        ...
        await batch.commit()
            ...
            .catch(serverError => {
                const permissionError = new FirestorePermissionError({
                    path: 'batch-write',
                    operation: 'write',
                    requestResourceData: { statement: newStatementData }
                });
                errorEmitter.emit('permission-error', permissionError);
                 toast({ variant: 'destructive', title: 'Error', description: 'Could not generate statement due to permissions.' });
