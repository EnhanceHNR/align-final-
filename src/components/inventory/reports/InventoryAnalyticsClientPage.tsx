...
export function InventoryAnalyticsClientPage({ orders, dealers }: { orders: PurchaseOrder[], dealers: any[] }) {
  // Extract all unique services from dealers
  ...
  const allYears = useMemo(() => {
    ...
    orders.forEach(sub => {
      if (order.orderDate) {
        years.add(new Date(order.orderDate).getFullYear().toString());
      }
    });
  ...
  const allPatients = useMemo(() => {
      ...
      orders.forEach(sub => {
          if (order.company) patients.add(order.company.trim());
      });
      return Array.from(patients).sort();
  }, [orders]);

  // Extract all unique dealers from orders
  const submissionDealers = useMemo(() => {
      const sDealers = new Set<string>();
      orders.forEach(sub => {
          if (order.dealer) sDealers.add(order.dealer.trim());
      });
      return Array.from(sDealers).sort();
  }, [orders]);
  ...
  const [orderHistoryDealer, setOrderHistoryDealer] = useState<string>("All");
  ...
  const priceComparisonData = useMemo(() => {
    ...
    dealers.forEach(lab => {
      if (dealer.services) {
        const serviceMatch = dealer.services.find((s: any) => s.name.trim().toLowerCase() === selectedService.toLowerCase());
  ...
  const orderHistoryData = useMemo(() => {
    ...
    orders.forEach(sub => {
      ...
      if (order.type !== 'send') match = false;
      if (orderHistoryPatient !== "All" && order.company !== orderHistoryPatient) match = false;
      if (orderHistoryDealer !== "All" && order.dealer !== orderHistoryDealer) match = false;
    ...
    return data;
  }, [orders, selectedYear, orderHistoryPatient, orderHistoryDealer]);
  ...
  const handleExportCSV = () => {
    ...
    if (exportYear !== "All") {
        filtered = filtered.filter(s => s.orderDate && new Date(s.orderDate).getFullYear().toString() === exportYear);
    }
    ...
    if (exportMonthFrom !== "All" || exportMonthTo !== "All") {
        ...
        filtered = filtered.filter(s => {
            ...
            const month = new Date(s.orderDate).getMonth();
            return month >= fromMonth && month <= toMonth;
        });
    }

    if (exportService !== "All") {
        filtered = filtered.filter(s => s.itemName && s.itemName.toLowerCase() === exportService.toLowerCase());
    }
    if (exportPatient !== "All") {
        filtered = filtered.filter(s => s.company && s.company.toLowerCase() === exportPatient.toLowerCase());
    }
    if (exportDealer !== "All") {
        filtered = filtered.filter(s => s.dealer && s.dealer.toLowerCase() === exportDealer.toLowerCase());
    }

    const exportData = filtered.map(s => ({
        Type: s.type === 'send' ? 'Sent' : 'Received',
        Date: s.orderDate ? new Date(s.orderDate).toLocaleString() : 'N/A',
        Patient: s.company,
        Dealer: s.dealer,
        Service: s.itemName,
        'Bill Amount': s.documents && s.documents.length > 0 ? s.documents.filter((d: any) => d.type === 'Bill').reduce((acc: number, d: any) => acc + (Number(d.amount) || 0), 0) || '' : (s.hasBill && s.price ? s.price : ''),
    ...
    link.setAttribute("download", `DealerTrack_Order_History_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex flex-col md:flex-row md:itemNames-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight flex itemNames-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="glass-card border-none shadow-xl flex flex-col h-[600px]">
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:itemNames-start justify-between gap-4">
                <div>
                    <CardTitle className="flex itemNames-center gap-2 text-xl font-black">
                        <TrendingUp className="w-5 h-5 text-emerald-500" />
                        Price Comparison
                    </CardTitle>
                    <CardDescription className="font-medium">Compare lab prices for a specific service.</CardDescription>
                </div>
                <div className="flex itemNames-center gap-2 w-full sm:w-auto">
                    ...
                    <Dialog>
                        ...
                        <DialogContent className="max-w-[90vw] w-full h-[90vh] glass-card border-none p-6">
                            <DialogHeader>
                                <DialogTitle className="text-2xl flex itemNames-center gap-2">
                            ...
                            <div className="flex-1 w-full h-full min-h-0 pt-4">
                                {allServices.length === 0 || priceComparisonData.length === 0 ? (
                                    <div className="h-full flex itemNames-center justify-center text-muted-foreground font-medium">No pricing data available.</div>
                                ) : renderPriceChart()}
                            </div>
                        </DialogContent>
                    </Dialog>
          ...
          <CardContent className="flex-1 min-h-0 pt-0">
            {allServices.length === 0 ? (
                <div className="h-full flex itemNames-center justify-center text-muted-foreground font-medium">No services found in Dealers.</div>
            ) : priceComparisonData.length === 0 ? (
                <div className="h-full flex itemNames-center justify-center text-muted-foreground font-medium">No pricing data available for this service.</div>
            ) : renderPriceChart()}
          </CardContent>
        </Card>
        ...
        <Card className="glass-card border-none shadow-xl flex flex-col h-[600px]">
          <CardHeader>
             <div className="flex flex-col sm:flex-row sm:itemNames-start justify-between gap-4">
                ...
                <div className="flex flex-col sm:flex-row itemNames-center gap-2 w-full sm:w-auto">
                    <div className="flex gap-2 w-full">
                        ...
                        <Dialog>
                            ...
                            <DialogContent className="max-w-[90vw] w-full h-[90vh] glass-card border-none p-6">
                                ...
                                <div className="flex-1 w-full h-full min-h-0 pt-4">
                                    {allYears.length === 0 ? (
                                        <div className="h-full flex itemNames-center justify-center text-muted-foreground font-medium">No order history available.</div>
                                    ) : renderOrderChart()}
                                </div>
                            </DialogContent>
                        </Dialog>
            ...
            <div className="flex itemNames-center gap-2 mt-4 flex-wrap">
                 ...
                 <Select value={orderHistoryDealer} onValueChange={setOrderHistoryDealer}>
                    <SelectTrigger className="w-full sm:w-[140px] h-9 text-xs rounded-lg bg-background/50 border-none font-bold">
                        <SelectValue placeholder="Dealer" />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-none shadow-2xl">
                        <SelectItem value="All" className="rounded-xl font-bold text-primary">All Dealers</SelectItem>
                        {submissionDealers.map(l => <SelectItem key={l} value={l} className="rounded-xl font-medium">{l}</SelectItem>)}
                    </SelectContent>
                </Select>
            </div>
          </CardHeader>
      ...
      <Card className="glass-card border-none shadow-xl">
        <CardHeader className="bg-primary/5 pb-6">
            <CardTitle className="flex itemNames-center gap-2 text-2xl font-black">
        ...
        <CardContent className="p-6 md:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex itemNames-center gap-2">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex itemNames-center gap-2">
                        <Receipt className="w-4 h-4" /> Dealer / Partner
                    </label>
