...
import { Delivery, InventoryTransaction } from "@/lib/types";
...
import { 
    ...
    Wallet, CreditCard, AlertCircle, CalendarIcon
} from "lucide-react";
...
import { CameraModal } from "@/components/lab/CameraModal";
...
export function InventoryPaymentsClientPage({ 
    deliveries, 
    transactions 
}: { 
    deliveries: Delivery[], 
  ...
  const dealerAccounts = useMemo(() => {
    ...
    const filteredDeliverys = deliveries.filter(sub => {
    ...
    // Process Deliverys as Bills
    filteredDeliverys.forEach((sub) => {
      const type = sub.documentType || (sub.billPhotoUrl ? 'Bill' : 'None');
      if (type !== 'None') {
        if (!accounts[sub.dealerName]) {
          accounts[sub.dealerName] = { totalBilled: 0, totalPaid: 0, ledger: [] };
        }
        
        const amount = Number(sub.actualPrice) || 0;
        
        if (type !== 'Challan') {
            accounts[sub.dealerName].totalBilled += amount;
        }
        
        accounts[sub.dealerName].ledger.push({
            id: sub.id,
            date: sub.createdAt,
            type: type as any,
            amount: amount,
            description: `${sub.itemPhotoUrl} - ${sub.id}`,
            item: sub.itemPhotoUrl,
            patientName: sub.id,
        ...
        if (sub.paymentStatus === 'Paid' && type !== 'Challan') {
            accounts[sub.dealerName].totalPaid += amount;
        }
      }
      ...
      if (sub.documents && sub.documents.length > 0) {
        ...
        sub.documents.forEach((doc, idx) => {
            ...
            accounts[sub.dealerName].ledger.push({
                ...
                amount: amount,
                description: `${sub.itemPhotoUrl} - ${sub.id} (${doc.type})`,
                item: sub.itemPhotoUrl,
                patientName: sub.id,
    ...
    filteredTransactions.forEach((tx) => {
        if (!accounts[tx.dealerName]) {
            accounts[tx.dealerName] = { totalBilled: 0, totalPaid: 0, ledger: [] };
        }

        accounts[tx.dealerName].ledger.push({
            id: tx.id,
            date: tx.createdAt,
            type: tx.type,
            amount: tx.amount,
        ...
        if (tx.amount > 0) {
            accounts[tx.dealerName].totalBilled += tx.amount;
        } else {
            accounts[tx.dealerName].totalPaid += Math.abs(tx.amount);
        }
    });
  ...
  }, [deliveries, transactions, fromDate, toDate]);
  ...
  const handleMarkAsPaid = async (submissionId: string, proofFile?: File) => {
    ...
    try {
      ...
      const result = toast({title: "Updated", description: "Mock payment status updated"});
      if (result.success) {
  ...
  const handleAddTransaction = async () => {
    if (!selectedDealer || !txAmount) return;
    setIsAddingTx(true);
    try {
        const amount = Number(txAmount);
        
        // Validation to prevent payment exceeding due amount
        const account = dealerAccounts.find(a => a.dealerName === selectedDealer);
        ...
        const formData = new FormData();
        formData.append('dealerName', selectedDealer);
        ...
        const result = await addInventoryTransactionAction(formData);
  ...
  const generatePDF = (account: any) => {
    const doc = new jsPDF();
    const dealerName = account.dealerName;
    ...
    doc.save(`LabTrack_${dealerName.replace(/\s+/g, '_')}_Statement.pdf`);
    toast({ title: "PDF Generated", description: `Statement for ${dealerName} downloaded.` });
  };
      ...
      {dealerAccounts.length === 0 ? (
        ...
        <div className="grid gap-6">
          {dealerAccounts.map((account) => (
            <Card key={account.dealerName} className="overflow-hidden border-none shadow-lg bg-white/50 dark:bg-white/5 ring-1 ring-border/5">
              <CardHeader className="bg-primary/5 pb-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-2xl font-black tracking-tight">{account.dealerName}</CardTitle>
                  ...
                  <div className="flex flex-wrap items-center gap-4 sm:gap-8">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button 
                            ...
                            onClick={() => {
                                setSelectedDealer(account.dealerName);
                                setTxType('Payment');
        ...
        <DialogContent className="sm:max-w-[425px] rounded-3xl overflow-hidden p-0 gap-0 border-none shadow-2xl">
          <DialogHeader className="bg-primary/5 p-6 md:p-8">
            ...
            <DialogDescription className="text-base font-medium">
                Add a payment or adjustment for <strong>{selectedDealer}</strong>
            </DialogDescription>
          </DialogHeader>
