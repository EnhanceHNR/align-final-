...
export function SendForm({ users = [], receivedRecords = [] }: SendFormProps) {
  ...
  useEffect(() => {
    form.register("senderSelfie");
    form.register("productPhotos");
  ...
  const nextStep = async () => {
    ...
    if (step === 3) fieldsToValidate = ["senderName"];

    const isValid = await form.trigger(fieldsToValidate);
    
    // Manually validate custom file states that RHF struggles to track when unmounted
    if (step === 3) {
        let hasFileError = false;
        if (!selfieFile) {
            form.setError("senderSelfie", { type: "manual", message: "Sender selfie is required." });
            hasFileError = true;
        } else {
            form.clearErrors("senderSelfie");
        }
        
        if (productFiles.length === 0) {
            form.setError("productPhotos", { type: "manual", message: "At least one product photo is required." });
            hasFileError = true;
        } else {
            form.clearErrors("productPhotos");
        }
    ...
    <div className="max-w-2xl mx-auto mt-4 px-2 sm:px-0 relative">
      ...
      <form 
        onSubmit={form.handleSubmit(onSubmit, (errors) => {
          ...
          toast({
            title: "Validation Error",
            description: "Please check all steps for missing required fields.",
            variant: "destructive"
          });
        })} 
