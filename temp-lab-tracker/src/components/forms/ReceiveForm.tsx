...
import { receiveSubmissionAction } from "@/app/actions";
...
export function ReceiveForm({ users = [], sentRecords = [] }: ReceiveFormProps) {
  ...
  useEffect(() => {
    ...
    async function loadEntities() {
        try {
            const { fetchEntitiesAction } = await import('@/app/actions');
            ...
            {appointmentStatus === 'Appointment given' && (
                <div className="animate-in fade-in slide-in-from-top-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel className="font-bold text-xs uppercase tracking-widest text-muted-foreground">Appointment Date & Time</FormLabel>
                                <FormControl>
                                    <Input 
                                        type="datetime-local" 
                                        {...field} 
